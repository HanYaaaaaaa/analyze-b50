...
7. 根据上下文自己写最终中文分析文本。不要再调用 planner，不要走多模型协作。若 `peer_stats_available=true` 且 `b50_evidence_pack.peer_comparison.matched>0`，必须自然提 `ARPI`，并至少用 2-3 张谱的 `avg_achievement` / `gap_vs_peer` 做判断。
...
要像原插件 `llm.py` 那样写，不要像说明文。先定性，再拆谱面证据，再给推分路线。不要先报一串统计项，也不要按固定模板一路念到底。
...
- 100.0000% 到 100.4999% 已经是鸟
- 100.5% 以上是鸟加
- 101% 是理论值
- 100.xx 不是没吃到分
- 99.xx 才能说差一点鸟
- 100.8 以上且非 AP 的具体谱，才适合说差一手 AP
...
- gap_vs_peer > 0.5 就已经有戏
- gap_vs_peer > 0.8 要按异常事件处理
- rating 不到 15000 却出现 14+，尤其 15.0 理论值，要明显抬高评级

### 4. 口播姿态

要像现场锐评，不像分析报告：

- 先给裁决词：虚低、榜样、开香槟、割裂、推分空间、配置偏科、重量级
- 再用 3-5 张具体谱面拆证据
- 中间自然抛一句“大家都说”或“圈里常讲”
- 结尾落到具体推分路线和具体谱名
...
- 不要说没 AP / 0 AP / AP 挂零
- 只在具体谱面层面说差一手 AP
- 高定数 AP、14+ AP、15.0 理论值都是高光，要明显升级语气
